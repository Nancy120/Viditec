...
├── src/
│   └── server.js              # Minimal Express + SQLite service fixture
├── fixtures/
│   ├── global-setup.js        # Starts server before test run
│   ├── global-teardown.js     # Stops server after run
│   └── wallet-fixture.js      # Playwright fixture: auto-reset DB + seed wallets
├── utils/
│   ├── api-client.js          # Thin API wrapper (all transport logic here)
│   └── data-builders.js       # Request builders, ID generators
├── tests/
│   ├── api/
│   │   └── api-contract.spec.js     # Contract, validation, error codes
│   ├── db/
│   │   └── db-persistence.spec.js   # DB invariants, row correctness
│   ├── e2e/
│   │   └── e2e-flow.spec.js         # Full flow + idempotency semantics
│   └── concurrency/
│       └── concurrency.spec.js      # Parallel requests, race conditions
└── playwright.config.js
```

Each test starts with a fresh DB (auto via `freshDB` fixture). Tests are isolated, deterministic, and repeatable.
